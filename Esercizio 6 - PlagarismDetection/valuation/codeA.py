...
def lesk_algorithm(word, sentence):
    ...
    for ss in wn.synsets(word):
        ...
        overlap = computeOverlap(signature.split(), context)
...
def load_dataset():
    ...
    # print(dataset)
    #a = list(map(str, semcor.tagged_chunks(tag='both')[:8]))

    # Conta quante volte il nostro algoritmo di lesk trova il synset corretto
    lesk_correct = 0

    for i in range(0, 50):  # per ogni frase
        print("-------------")
        # associa il senso di wordnet ad ogni parola della frase i
        lemma = lemma_list(semcor.tagged_sents(tag="sem")[i])
        # print(lemma)
        index = len(lemma)
        ...
        for j in range(0, index):
            if (found == False):
                if (str(lemma[j]) != 'None'):
                    string = str(lemma[j])  # prendo la parola e la pulisco
                    string = string.replace("Lemma('", "")
                    string = string.replace("')", "")
                    parts = string.split('.')
                    # Lemma('recommend.v.01.recommend') --> "synset".paroladeltesto
                    # parts[0] = reccommend
                    # parts[1] = v
                    # parts[2] = 01
